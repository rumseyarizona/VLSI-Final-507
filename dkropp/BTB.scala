...
object BHTType {
  val gshare        = 0  // PC indexed table w/ history
  val bimodal       = 1  // PC indexed table, no history
  val staticNotTaken= 2  // not tanke
  val staticTaken   = 3  // taken
  val neural        = 4  // perceptron
}

// config sets which BHT algorithm to use.

case object BHTTypeKey extends Field[Int]
...
// Common interface for branch history tables
trait BHTBase {
...
class BHT(params: BHTParams)(implicit val p: Parameters)
  extends HasCoreParameters with BHTBase {
  // predictor chosen at elaboration time
  private val bhtType          = p(BHTTypeKey)
  private val useGShare        = bhtType == BHTType.gshare
  private val useStaticNotTaken= bhtType == BHTType.staticNotTaken
  private val useStaticTaken   = bhtType == BHTType.staticTaken
  private val useBimodal        = bhtType == BHTType.bimodal

  //  for gshare: PC XOR history
  //  for bimodal/static: PC only
  def index(addr: UInt, history: UInt) = {
    ...
    if (useGShare) {
      ...
      // bimodal or static-not-taken or static-taken: just use address
      hashAddr(addr)
    }
  }
  ...
  def get(addr: UInt): BHTResp = {
    ...
    if (useStaticNotTaken) {
      res.value   := 0.U
      res.history := 0.U
    } 

    else if (useStaticTaken) {
      res.value   := ((1 << params.counterLength) - 1).U(params.counterLength.W)
      res.history := 0.U
    } 
    
    else {
      res.value   := Mux(resetting, 1.U, table(index(addr, history)))
      res.history := history
    }
  ...
  // update table 
  def updateTable(addr: UInt, d: BHTResp, taken: Bool): Unit = if (!useStaticNotTaken && !useStaticTaken) {
    wen := true.B
    when (!resetting) {
  ...
  // history update/reset
  def resetHistory(d: BHTResp): Unit = if (!useStaticNotTaken && !useStaticTaken) {
    history := d.history
  }
  def updateHistory(addr: UInt, d: BHTResp, taken: Bool): Unit = if (!useStaticNotTaken && !useStaticTaken) {
    history := Cat(taken, d.history >> 1)
  }
  def advanceHistory(taken: Bool): Unit = if (!useStaticNotTaken && !useStaticTaken) {
...
class NeuralBHT(params: BHTParams)(implicit val p: Parameters)
  extends HasCoreParameters with BHTBase {

  val nEntries      = params.nEntries
  val histLen       = params.historyLength
  val weightBits    = 8 
  val theta         = (1.93 * histLen + 14.0).toInt

  // PC hashing
  private def hashAddr(addr: UInt): UInt = {
    val hi = addr >> log2Ceil(fetchBytes)
    hi(log2Ceil(nEntries)-1, 0) ^ (hi >> log2Ceil(nEntries))(1, 0)
  }

  // global history
  val history = RegInit(0.U(histLen.W))

  // table of perceptrons
  val table = Mem(nEntries, Vec(histLen + 1, SInt(weightBits.W)))

  // reset logic
  private val reset_waddr = RegInit(0.U((log2Ceil(nEntries)+1).W))
  private val resetting   = !reset_waddr(log2Ceil(nEntries))
  when (resetting) {
    val zeroVec = VecInit(Seq.fill(histLen + 1)(0.S(weightBits.W)))
    table(reset_waddr) := zeroVec
    reset_waddr := reset_waddr + 1.U
  }

  private def xFromHistory(hist: UInt): Vec[SInt] = {
    // convert history bits {0,1} -> {-1, +1}
    VecInit((0 until histLen).map { i =>
      Mux(hist(i), 1.S(weightBits.W), (-1).S(weightBits.W))
    })
  }
  ...
  def get(addr: UInt): BHTResp = {
    ...
    val weights = table(idx)

    val bias    = weights(0)
    val xVec    = xFromHistory(history)

    // sum = bias + Î£ w_i * x_i
    val sum = xVec.zip(weights.tail).map { case (x, w) => w * x }.foldLeft(bias)(_ + _)

    val taken = sum >= 0.S

    res.history := history
    res.value   := Mux(taken, 1.U, 0.U) 
  ...
  def updateTable(addr: UInt, d: BHTResp, taken: Bool): Unit = {
    when (!resetting) {
      val idx      = hashAddr(addr)
      val weights  = table(idx)
      val xVec     = xFromHistory(d.history)

      val t           = Mux(taken, 1.S(weightBits.W), (-1).S(weightBits.W))
      val yPredTaken  = d.taken

      // train only on mispredicts
      when (taken =/= yPredTaken) {
        val newVec = Wire(Vec(histLen + 1, SInt(weightBits.W)))

        // update bias
        newVec(0) := saturatingAdd(weights(0), t)

        // update history weights
        for (i <- 0 until histLen) {
          newVec(i+1) := saturatingAdd(weights(i+1), t * xVec(i))
        }
  ...
  //helper for saturating add
  private def saturatingAdd(a: SInt, b: SInt): SInt = {
    val sum    = a +& b
    val maxVal = ((1 << (weightBits-1)) - 1).S
    val minVal = (-(1 << (weightBits-1))).S
    Mux(sum > maxVal, maxVal, Mux(sum < minVal, minVal, sum.asSInt))
  }
...
class BTB(implicit p: Parameters) extends BtbModule {
  ...
  if (btbParams.bhtParams.nonEmpty) {
    // choose which BHT implementation to elaborate based on BHTTypeKey
    val bht: BHTBase =
      if (p(BHTTypeKey) == BHTType.neural) {
        new NeuralBHT(btbParams.bhtParams.get)
      } else {
        new BHT(btbParams.bhtParams.get)
      }
